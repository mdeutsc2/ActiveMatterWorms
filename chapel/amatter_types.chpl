...
var bound: [numPoints] Particle;
var solvent: [numPoints] Particle;
...
proc init_worms() {
    ...
    if (boundary == 1){
        ...
        for i in 1..numPoints {
            bound[i].x = ca * (1 - cos(thetaValues[i])) * cos(thetaValues[i]) + hxo2;
    ...
    for iw in 1..nworms {
        if (ireverse[iw] == 1) {
            ...
            for ip in 1..np {
                worms[iw.i].x = savex[np+1-ip];
                worms[iw,i].y = savey[np+1-ip];
            }
        }
    }
    writeln("init done");
