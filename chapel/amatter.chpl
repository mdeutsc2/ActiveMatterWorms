...
proc update_pos(itime:int) {
    ...
    for iw in 1..nworms {
       for i in 1..np {
            ...
            fyold[iw, i] = fy[iw, i];
            //checking for nan
            //if (isnan(fx[iw,i])) {halt("fx is nan");}
            //if (isnan(fy[iw,i])) {halt("fy is nan");}
            //if (isnan(vx[iw,i])) {halt("vx is nan");}
            //if (isnan(vy[iw,i])) {halt("vy is nan");}
            if (isnan(x[iw,i])) {
                writeln(x[iw,i],"\t",y[iw,i],"\t",vx[iw,i],"\t",vy[iw,i],"\t",fx[iw,i],"\t",fy[iw,i]);
                halt("x is nan");
            }
            if (isnan(y[iw,i])) {
                writeln(x[iw,i],"\t",y[iw,i],"\t",vx[iw,i],"\t",vy[iw,i],"\t",fx[iw,i],"\t",fy[iw,i]);
...
proc worm_wall() {
    ...
    //forall iw in 1..nworms {
    for iw in 1..nworms {
        var dx:real,dy:real,r:real,r2:real,th:real,xwall:real,ywall:real,rr2:real,ffor:real,
        dxi:real,dyi:real,ri:real,dxj:real,dyj:real,ffx:real,ffy:real;
        var ip1:int;
        for i in 1..np {
            // dissipation proportional to v relative to local average
            fx[iw,i] -= diss*vx[iw,i] - vxave[iw,i];
            fy[iw,i] -= diss*vy[iw,i] - vyave[iw,i];
            if (isnan(fx[iw,i])) {
                halt();
            } else if (isnan(fy[iw,i])) {
                halt();
            }
            ...
            if (r2 >= r2inside) {
               ...
               if (iwalldrive == 1) {
                ...
                dxi = dxi/ri;
                dyi = dyi/ri;
                // calculate unit vector along the wall
                dxj = -sin(th);
                dyj = cos(th);
                ...
                // if the two vectors have any component points in opposite directions
                if ((dxi*dxj + dyi*dyj) < 0.0) {
...
proc cell_sort(itime:int) {
    ...
    for iworm in 1..nworms {
        for ip in 1..np {
            //if ((iworm == 1076) && (40 <= ip <= 49)){
                writeln(itime,"\t",iworm,"\t",ip,"\t",x[iworm,ip],"\t",y[iworm,ip],"\t",vx[iworm,ip],"\t",vy[iworm,ip],"\t",fx[iworm,ip],"\t",fy[iworm,ip]);
            //}
            ii = (iworm-1)*np + ip;
            icell = 1+floor(x[iworm,ip]/dcell):int;
            jcell = 1+floor(y[iworm,ip]/dcell):int;
            if ((icell > nxcell) || (icell < 1)) {
                writeln("nxcell=",nxcell," icell=",icell);
                writeln(x[iworm,ip]/dcell);
                writeln(floor(x[iworm,ip]/dcell));
    ...
    for icell in 1..nxcell {
        for jcell in 1..nycell {
            ...
            if (hhead[scell] != -1) {
                // there are particles in the cell called scell, so lets check all the neighboring cells
                for idir in 1..9 {
                    icnab = icell + ddx[idir];
                    if (icnab > nxcell || icnab == 0) {
                        break;
                    }
                    jcnab = jcell + ddy[idir];
                    if (jcnab > nycell || jcnab == 0) {
                        break;
                    } 
                    scnab = icnab + (jcnab - 1)*nxcell; //1d neighbor
                    if (hhead[scnab] == -1) {
                        break; // no particles in the cell called scnab
                    }
                    ...
                    while (ii > 0) {
                        ...
                        jj = hhead[scnab]; // head particle of the neighboring cell
                        while (jj > 0) {
                            jworm = 1+((jj-1)/np):int;
                            jp = jj - np*(jworm-1);
                            inogo = 0;
                            if (iworm == jworm && abs(ip-jp) <= 2) {
                                inogo = 1; //on the same worm and too close means no interaction here
                            }
                            if (ii < jj && inogo == 0) {
                                ...
                                if (r2 <= r2cutsmall) {
                                    ...
                                    vxave[iworm,ip] += vx[jworm,jp];
                                    vyave[iworm,ip] += vy[jworm,jp];
                                    nnab[iworm,ip] += 1;
                                    vxave[jworm,jp] += vx[iworm,ip];
                                    vyave[jworm,jp] += vy[iworm,ip];
                                    nnab[jworm,jp] += 1;
                                    ...
                                    if (dxi*dxj + dyi*dyj <= 0.0) {
                                        ...
                                        dxj = dxj/rj;
                                        dyj = dyj/rj;
                                        // now that they are both unit vectors, find the direction for the force
                                        dx = (dxi-dxj)/2.0;
                                        dy = (dyi-dyj)/2.0;
                                        ...
                                        if (isnan(ffx)) {
                                            writeln(ffx);
                                            writeln(fdogic,"\t",dx,"\t",dddx,"\t",riijj);
                                            writeln(r,"\t",dxi,"\t",dxj);
                                            halt();
                                        } else if (isnan(ffy)) {
                                            writeln(ffy);
                                            writeln(fdogic,"\t",dy,"\t",dddy,"\t",riijj);
                                            writeln(r,"\t",dyi,"\t",dyj);
                                            halt();
                                        }
...
proc update_vel() {
    for iw in 1..nworms {
        for i in 1..np {
            ...
            if (isnan(vx[iw,i])) {
                halt();
            } else if(isnan(vy[iw,i])) {
                halt();
            } else if (isnan(fx[iw,i])) {
                halt();
            } else if (isnan(fy[iw,i])) {
                halt();
            } else if (isnan(fxold[iw,i])) {
                halt();
            } else if (isnan(fyold[iw,i])) {
                halt();
            }
        }
    }
