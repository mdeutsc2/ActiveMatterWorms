...
var worms_h: [1..nworms,1..np] Structs.Particle;
Structs.ptc_init_counter = 1;

var numSol:int;
numSol = init_fluid_count();
var solvent_h: [1..numSol] Structs.Particle;
Structs.ptc_init_counter = 1;

var bound_h: [1..numPoints] Structs.Particle;
Structs.ptc_init_counter = 1;
...
proc main() {
    //init_log(logfile);
    writeln("=============== AMATTER3D ===============");
    writeln(date.today():string+":Matt Deutsch, Kent State University");
   //  writeln("starting... "+numTasks:string+" procs");
    ...
    for iw in 1..nworms {
        for i in 1..np {
            worms_h[iw,i].m = worm_particle_mass;
        }
    }

    if (fluid_cpl) {
        solvent_h = init_fluid(solvent_h, numSol);
    }
    // populate the bins with lists of atoms
    update_cells(0);
    writeln("numSol\t"+numSol:string);
    writeln("fluid equilibrated...5000dt");
   // initialize the GPU calculations
   writeln((+ reduce worms_h.x));
   on device {
      // declare device variables
      var worms_d: [1..nworms,1..np] Structs.Particle;
      Structs.ptc_init_counter = 1;
      var solvent_d: [1..numSol] Structs.Particle;
      Structs.ptc_init_counter = 1;
      var bound_d: [1..numPoints] Structs.Particle;
      Structs.ptc_init_counter = 1;
      worms_d = worms_h;
      solvent_d = solvent_h;
      bound_d = bound_d;

      var bins_d : [1..numBins*numBins] Structs.Bin;
      var binSpaceiodd_d : [1..(numBins*numBins)/2] int;
      var binSpacejodd_d : [1..(numBins*numBins)/2] int;
      var binSpaceieven_d : [1..(numBins*numBins)/2] int;
      var binSpacejeven_d : [1..(numBins*numBins)/2] int;
      bins_d = bins;
      binSpaceiodd_d = binSpaceiodd;
      binSpacejodd_d = binSpacejodd;
      binSpaceieven_d = binSpaceieven;
      binSpacejeven_d = binSpacejeven;
      on host {
      ...
      for itime in 1..nsteps {
         ...
         if (itime % io_interval == 0) {
            on host {
               ...
               var out_str:string = "Step: "+itime:string+"\t"+
                      (io_interval/xt.elapsed()):string+"iter/s\tElapsed"+
         ...
         //update_pos(worms_d);
         forall i in 1..nworms*np {
            var iw = 1 + ((i - 1)/np):int; // find which worm j is in
            var ip = i - np*(iw - 1); // which particle in the worm is j?
            worms_d[iw,ip].fx /= worms_d[iw,ip].m;
            worms_d[iw,ip].fy /= worms_d[iw,ip].m;
            worms_d[iw,ip].fz /= worms_d[iw,ip].m;
            // worms_d[iw,ip].fx = worms_d[iw,ip].fx/worms_d[iw,ip].m;
            // worms_d[iw,ip].fy = worms_d[iw,ip].fy/worms_d[iw,ip].m;
            // worms_d[iw,ip].fz = worms_d[iw,ip].fz/worms_d[iw,ip].m;
      //}
            ...
            worms_d[iw,ip].x = worms_d[iw,ip].x + worms_d[iw,ip].vx*dt + worms_d[iw,ip].fx*dt2o2;
            worms_d[iw,ip].y = worms_d[iw,ip].y + worms_d[iw,ip].vy*dt + worms_d[iw,ip].fy*dt2o2;
            worms_d[iw,ip].z = worms_d[iw,ip].z + worms_d[iw,ip].vz*dt + worms_d[iw,ip].fz*dt2o2;

            worms_d[iw,ip].fxold = worms_d[iw,ip].fx;
            worms_d[iw,ip].fyold = worms_d[iw,ip].fy;
            worms_d[iw,ip].fzold = worms_d[iw,ip].fz;

            worms_d[iw,ip].fx = 0.0;
            worms_d[iw,ip].fy = 0.0;
            worms_d[iw,ip].fz = 0.0;

            // // periodic boundary conditions top and bottom for worms
            // //periodic boundary conditions
            // if worms_d[iw,ip].z > L {
            //       //worms[iw,i].z = worms[iw,i].z - L;
            //       worms_d[iw,ip].z = L;
            // } else if worms_d[iw,ip].z < 0.0 {
            //       //worms[iw,i].z = worms[iw,i].z + L;
            //       worms_d[iw,ip].z = 0.0;
            // }
         }
         ...
         //update_cells_gpu(itime,worms_d,solvent_d,bound_d,bins_d);
   ...
   halt();
    ...
    xt.stop();
    //write_macro(nsteps);
    writeln("Total Time:"+total_time:string+" s");
}
...
proc init_worms() {
    ...
    var rmin = a*thetanow;
    writeln("nworms\t"+nworms:string);
    writeln("np\t"+np:string);
    writeln("rwall\t"+rwall:string);
    writeln("nxcells\t"+nxcell:string+"\t nycells\t"+nycell:string);
    writeln("density\t"+density:string);
    writeln("dt\t"+dt:string);
    writeln("opt numPoints\t"+(ceil(2*pi*rwall)/rcutsmall):string);
    ...
    if (boundary == 1){
        ...
        for i in 1..numPoints {
            bound_h[i].x = rwall * cos(equidistantThetaValues[i])+hxo2;
            bound_h[i].y = rwall * sin(equidistantThetaValues[i])+hyo2;
            bound_h[i].z = 0.0;
            bound_h[i].ptype = 3;
        }
        for iw in 1..nworms {
            ...
            for i in 1..np {
                ...
                dth = length0/r;
                thetanow += dth;
                worms_h[iw,i].x = hxo2 + r*cos(thetanow);
                worms_h[iw,i].y = hyo2 + r*sin(thetanow);
                worms_h[iw,i].z = randStream.getNext()*(L);
                xangle = atan2(worms_h[iw,i].y - hyo2, worms_h[iw,i].x - hxo2);
                //TODO give them an initial velocity going around the circle
                worms_h[iw,i].ptype = 1;
                worms_h[iw,i].m = 1.0; // setting mass
                // vx[iw,i] = 0.0; // NOTE: this is all set by initializer
                // vy[iw,i] = 0.0;
                worms_h[iw,i].vxave = 0.0;
                worms_h[iw,i].vyave = 0.0;
                worms_h[iw,i].vzave = 0.0;
                // fx[iw,i] = 0.0;
                // fy[iw,i] = 0.0;
                // fxold[iw,i] = 0.0;
                // fyold[iw,i] = 0.0;
   ...
   /*
        ...
        writeln("cardioid cirum: "+totalArcLength:string+"\t"+(totalArcLength/r2cutsmall):string);
        for i in 1..numPoints {
   ...
   */
    ...
    for iw in 1..nworms {
        if (ireverse[iw] == 1) {
            for i in 1..np {
                savex[i] = worms_h[iw,i].x;
                savey[i] = worms_h[iw,i].y;
                savez[i] = worms_h[iw,i].z;
            }
            for ip in 1..np {
                worms_h[iw,ip].x = savex[np+1-ip];
                worms_h[iw,ip].y = savey[np+1-ip];
                worms_h[iw,ip].z = savez[np+1-ip];
            }
        }
    }
...
inline proc update_pos(ref worms_d:[] Structs.Particle) {
...
proc worm_cell_forces(i:int,j:int) {
...
proc init_fluid(ref solvent: [] Structs.Particle,ref numSol: int) {
   ...
   if (boundary == 1) {
      ...
      for iw in 1..nworms {
            for i in 1..np {
               if (worms_h[iw,i].x > max_x) {
                  max_x = worms_h[iw,i].x;
               }
               if (worms_h[iw,i].y > max_y) {
                  max_y = worms_h[iw,i].y;
               }
            }
      }
      // circular boundary
...
// CELL LIST FUNCTIONS
proc update_cells_gpu(istep:int, ref worms_d: [] Structs.Particle,
                      ref solvent_d: [] Structs.Particle, 
                      ref bound_d: [] Structs.Particle,
                      ref bins_d: [] Structs.Bin) {
    // ncount array set to zero
    // particle list set to empty

    // resetting all other bins lists for worms and solvent
    foreach binid in binSpace{
        if bins_d[binid].ncount > 0 {
            bins_d[binid].ncount = 0; // number of boundary atoms should not change
            bins_d[binid].scount = 0;
            bins_d[binid].wcount = 0;
            bins_d[binid].bcount = 0;
            bins_d[binid].atoms.clear();
            bins_d[binid].types.clear();
        }
    }
    var ibin,jbin,binid :int;
    for i in 1..#numSol { // populating solvent particles into bins
        ibin=ceil(solvent_d[i].x/rcut):int;
        jbin=ceil(solvent_d[i].y/rcut):int;
        binid=(jbin-1)*numBins+ibin;
        if (binid > numBins*numBins) {
            writeln(solvent_d[i].info());
            sudden_halt(istep);
        } else if (binid < 1) {
            writeln(solvent_d[i].info());
            sudden_halt(istep);
        ...
        bins_d[binid].scount += 1;
        bins_d[binid].ncount += 1;
        bins_d[binid].atoms.pushBack(solvent_d[i].id);
        bins_d[binid].types.pushBack(solvent_d[i].ptype);
        //if (debug) {writeln(i,"\t",solvent[i].x,"\t",solvent[i].y,"\t",binposx,"\t",binposy);}
    }
    var wormid : int;
    for iw in 1..#nworms { // populating worm particles into bins
        for ip in 1..np {
            ibin=ceil(worms_d[iw,ip].x/rcut):int;
            jbin=ceil(worms_d[iw,ip].y/rcut):int;
            binid=(jbin-1)*numBins+ibin;
            wormid = (iw-1)*np+ip;
            if (binid > numBins*numBins) {
                writeln(worms_d[iw,ip].info());
                sudden_halt(istep);
            } else if (binid < 1) {
                writeln(worms_d[iw,ip].info());
                sudden_halt(istep);
            ...
            //append i to particle_list for binid
            bins_d[binid].wcount += 1;
            bins_d[binid].ncount += 1;
            bins_d[binid].atoms.pushBack(wormid);
            bins_d[binid].types.pushBack(worms_d[iw,ip].ptype);
            //if (debug) {writeln(i,"\t",solvent[i].x,"\t",solvent[i].y,"\t",binposx,"\t",binposy);}
        }
    }
    for ib in 1..numPoints {
        ibin=ceil(bound_d[ib].x/rcut):int;
        jbin=ceil(bound_d[ib].y/rcut):int;
        binid=(jbin-1)*numBins+ibin;
        //append i to particle_list for binid
        bins_d[binid].bcount += 1;
        bins_d[binid].ncount += 1;
        bins_d[binid].atoms.pushBack(bound_d[ib].id);
        bins_d[binid].types.pushBack(bound_d[ib].ptype);
    }
}
...
proc update_cells(istep:int) {
    ...
    var ibin,jbin,binid :int;
    for i in 1..numSol { // populating solvent particles into bins
        ibin=ceil(solvent_h[i].x/rcut):int;
        jbin=ceil(solvent_h[i].y/rcut):int;
        binid=(jbin-1)*numBins+ibin;
        if (binid > numBins*numBins) {
            writeln(solvent_h[i].info());
            sudden_halt(istep);
        } else if (binid < 1) {
            writeln(solvent_h[i].info());
            sudden_halt(istep);
        ...
        bins[binid].scount += 1;
        bins[binid].ncount += 1;
        bins[binid].atoms.pushBack(solvent_h[i].id);
        bins[binid].types.pushBack(solvent_h[i].ptype);
        //if (debug) {writeln(i,"\t",solvent[i].x,"\t",solvent[i].y,"\t",binposx,"\t",binposy);}
    }
    var wormid : int;
    for iw in 1..nworms { // populating worm particles into bins
        for ip in 1..np {
            ibin=ceil(worms_h[iw,ip].x/rcut):int;
            jbin=ceil(worms_h[iw,ip].y/rcut):int;
            binid=(jbin-1)*numBins+ibin;
            wormid = (iw-1)*np+ip;
            if (binid > numBins*numBins) {
                writeln(worms_h[iw,ip].info());
                sudden_halt(istep);
            } else if (binid < 1) {
                writeln(worms_h[iw,ip].info());
                sudden_halt(istep);
            ...
            bins[binid].types.pushBack(worms_h[iw,ip].ptype);
            //if (debug) {writeln(i,"\t",solvent[i].x,"\t",solvent[i].y,"\t",binposx,"\t",binposy);}
        }
    }
    for ib in 1..numPoints {
        ibin=ceil(bound_h[ib].x/rcut):int;
        jbin=ceil(bound_h[ib].y/rcut):int;
        binid=(jbin-1)*numBins+ibin;
        //append i to particle_list for binid
        bins[binid].bcount += 1;
        bins[binid].ncount += 1;
        bins[binid].atoms.pushBack(bound_h[ib].id);
        bins[binid].types.pushBack(bound_h[ib].ptype);
    }
}
...
proc write_xyz(istep:int) {
    ...
    ic = 2;
    for iw in 1..nworms {
        dx = worms_h[iw,1].x - hxo2;
        dy = worms_h[iw,1].y - hyo2;
        xang = atan2(dy,dx);
        rx = -sin(xang);
        ry = cos(xang);
        dot = (worms_h[iw,1].x - worms_h[iw,np].x)*rx + (worms_h[iw,1].y - worms_h[iw,np].y)*ry;
        if (dot >= 0.0) {
            for i in 1..np {
                myFileWriter.writeln("A ",worms_h[iw,i].x," ",worms_h[iw,i].y," ", worms_h[iw,i].z);
                //myFileWriter.writeln("A ",x[iw,i]," ",y[iw,i]," ", 0.0," ",vx[iw,i]," ",vy[iw,i]," ",0.0);
                ic += 1;
            }
        } else {
            for i in 1..np {
                myFileWriter.writeln("B ",worms_h[iw,i].x," ",worms_h[iw,i].y," ", worms_h[iw,i].z);
    ...
    for i in 1..numPoints {
        myFileWriter.writeln("I ",bound_h[i].x," ",bound_h[i].y," ",0.0);
        ic += 1;
    }
    if (fluid_cpl) {
        for i in 1..numSol {
            myFileWriter.writeln("S ",solvent_h[i].x," ",solvent_h[i].y," ",0.0);
            ic += 1;
        }
    }
...
proc write_xyzv(istep:int) {
    ...
    for iw in 1..nworms {
        ...
        if (dot >= 0.0) {
            for i in 1..np {
                myFileWriter.writeln("A ",worms_h[iw,i].x," ",worms_h[iw,i].y," ", worms_h[iw,i].z," ",worms_h[iw,i].vx," ",worms_h[iw,i].vy," ",worms_h[iw,i].vz);
                //myFileWriter.writeln("A ",x[iw,i]," ",y[iw,i]," ", 0.0," ",vx[iw,i]," ",vy[iw,i]," ",0.0);
                ic += 1;
            }
        } else {
            for i in 1..np {
                myFileWriter.writeln("B ",worms_h[iw,i].x," ",worms_h[iw,i].y," ",worms_h[iw,i].z," ",worms_h[iw,i].vx," ",worms_h[iw,i].vy," ",worms_h[iw,i].vz);
                //myFileWriter.writeln("B ",x[iw,i]," ",y[iw,i]," ", 0.0," ",vx[iw,i]," ",vy[iw,i]," ",0.0);
                ic += 1;
            }
        }
    ...
    for i in 1..numPoints {
        myFileWriter.writeln("I ",bound_h[i].x," ",bound_h[i].y," ",0.0," ",0.0," ",0.0," ",0.0);
        ic += 1;
    }
    if (fluid_cpl) {
        for i in 1..numSol {
            myFileWriter.writeln("S ",solvent_h[i].x," ",solvent_h[i].y," ",0.0," ",solvent_h[i].vx," ",solvent_h[i].vy," ",0.0);
            ic += 1;
        }
    }
    myFileWriter.writeln("E ",hxo2 - rwall," ",hyo2 - rwall," ",0.0," ",0.0," ",0.0," ",0.0);
    ...
    // myFileWriter.writeln("E ",hxo2 + rwall," ",hyo2 - rwall," ",0.0," ",0.0," ",0.0," ",0.0);
    // myFileWriter.writeln("E ",hxo2 + rwall," ",hyo2 + rwall," ",0.0," ",0.0," ",0.0," ",0.0);
    writeln(filename+"\t"+ic:string+" lines written",true);
    //xyzfile.close();
    } catch e: Error {
        writeln(e);
    }
