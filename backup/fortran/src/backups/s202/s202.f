	  ...
	  parameter(np=400,nworms=216,nsteps=2000000)
...
c         18a
	   ...
	   open(unit=10,file='s202a.xyz',status='unknown')
	   open(unit=16,file='s202a-debug.dat',status='unknown')
	    npm1=np-1
...
205	  continue
	  ...
	  if(mod(itime,5000).eq.0)then
	   open(unit=21,file='s202a.dat',status='unknown')
	    write(21,*)nworms,np
