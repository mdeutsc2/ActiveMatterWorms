...
!setting up dissipation relative to local average velocity summed over neighbors
...
cc               first calculate unit vector along the worm
            ip1=i+1
            if(ip1.le.np)then dxi=x(iw,ip1)-x(iw,i) dyi=y(iw,ip1)-y(iw,i)
        else dxi=x(iw,i)-x(iw,i-1) dyi=y(iw,i)-y(iw,i-1)
            endif
