program lbm
    ...
    ! ARRAYS
    real(real64),dimension(:,:),codimension[:],allocatable :: rho
    real(real64),dimension(:,:,:),codimension[:],allocatable :: vel
    integer(int64),dimension(:,:),codimension[:],allocatable :: mask
    real(real64),dimension(:,:,:),codimension[:],allocatable :: f_old
    real(real64),dimension(:,:,:),codimension[:],allocatable :: f_new
    real(real64),dimension(:,:,:),codimension[:],allocatable :: gather
    real(real64),dimension(9),codimension[*] :: w
    real(real64),dimension(2,9),codimension[*] :: e
    integer(int64),dimension(0:3),codimension[*] :: bc_type
    real(real64),dimension(2,0:3),codimension[*] :: bc_value 
    ! VARIABLES
    ...
    ! CO-ARRAY ALLOCATION
    integer(int64) :: indices(4),n
    integer(int32) :: igs,ige,jgs,jge ! global indices
    integer(int64) :: ils,ile,jls,jle ! local indices, excluding halo
    integer(int64) :: ims,ime,jms,jme ! local memory indices (includes halo)

    indices = tile_indices(nx,ny,.true.,.false.)
    indices(3) = 1
    indices(4) = ny
    do n = 1,num_images()
    if (this_image() == n) then
        print*, "Image #",n,indices,num_images()
    endif
    enddo
    igs = indices(1)
    ige = indices(2)
    ...
    do n = 1,num_images()
    if (this_image() == n) then
        print*, "Image #",n,ils,ile,jls,jle,ims,ime,jms,jme
    endif
    enddo

    allocate(rho(ims:ime,jms:jme)[*])
    allocate(vel(ims:ime,jms:jme,2)[*])
    allocate(mask(ims:ime,jms:jme)[*])
    allocate(f_old(9,ims:ime,jms:jme)[*])
    allocate(f_new(9,ims:ime,jms:jme)[*])
    allocate(gather(1:nx,1:ny,2)[*])
    ...
    contains
        ...
        pure function tile_indices(xdims,ydims,splitx,splity)
            integer(int64), intent(in) :: xdims,ydims
            logical, intent(in) :: splitx,splity
            integer(int64) :: tile_indices(4)
            integer(int64) :: offset, tile_size
            if (splitx) then
                tile_size = xdims/num_images()
                tile_indices(1) = (this_image() - 1) * tile_size + 1
                tile_indices(2) = tile_indices(1) + tile_size - 1
                offset = num_images() - mod(xdims,num_images())
            ...
            if (splity) then
                tile_size = ydims/num_images()
                tile_indices(3) = (this_image() - 1) * tile_size + 1
                tile_indices(4) = tile_indices(3) + tile_size - 1
                offset = num_images() - mod(xdims,num_images())
                if (this_image() > offset) then
                    tile_indices(3) = tile_indices(3) + this_image() - offset - 1
                    tile_indices(4) = tile_indices(4) + this_image() - offset
                endif
            endif
        end function tile_indices

        pure function tile_neighbors()
            integer(int64) :: tile_neighbors(4) !up,down,left,right
            integer(int64) :: up,down,left,right
