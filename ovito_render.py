...
def main(args):
    ...
    for f in args.filename:
        if args.worms:
            Render.worm_positions(f,"test_anim_worms.mp4")
            # Render.worm_positions(f,test_name)
        if args.solvent:
            Render.solvent_positions(f,"test_anim_solvent.mp4")
            # Render.solvent_positions(f,test_name)
        if args.all:
            Render.all_positions(f,"test_anim_all.mp4")
            # Render.all_positions(f,test_name)
        if args.velocity:
